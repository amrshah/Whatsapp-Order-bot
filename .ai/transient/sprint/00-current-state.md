...
9. **OAuth Social Authentication (Google/Facebook)**: Finished and fixed the OAuth sign-in and registration flow. Added social provider columns to the User model fillable attributes, resolved lowercase Spatie role mappings during registration callbacks, implemented unique slug generation for auto-onboarded tenants, configured dynamic absolute redirect callbacks based on `APP_URL`, and added Google/Facebook buttons to the registration card (`Register.jsx`). Added Pest integration tests (48 tests passing).
